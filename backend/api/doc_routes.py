...
async def get_document(doc_id: str, current_user: dict = Depends(get_current_user)):
    doc = await docs_collection.find_one({"_id": ObjectId(doc_id), "owner_id": str(current_user["_id"])})
    if not doc:
...
async def update_document(doc_id: str, doc_update: DocumentCreate, current_user: dict = Depends(get_current_user)):
    # Find the document and update its title, content, and updated_at time
    updated_doc = await docs_collection.find_one_and_update(
        {"_id": ObjectId(doc_id), "owner_id": str(current_user["_id"])},
        {"$set": {
            "title": doc_update.title,
            "content": doc_update.content,
            "updated_at": datetime.now(timezone.utc)
        }},
        return_document=True # Returns the newly updated document
    )
